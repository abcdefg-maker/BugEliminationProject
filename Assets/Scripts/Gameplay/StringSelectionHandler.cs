...
namespace BugElimination
{
    ...
    public class StringSelectionHandler : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        ...
        IEnumerator FadeOutCompletion()
        {
            ...
            // 切回办公室
            GameStateManager.Instance.RemoveFlag(GameConstants.Flags.CanEnterWorkScene);

            if (GameStateManager.Instance.currentDay == 1)
            {
                GameStateManager.Instance.SetFlag(GameConstants.Flags.CanTalkToWangDay1);
                SceneStateManager.Instance.ManualSave();
                SceneManager.LoadScene(GameConstants.Scenes.Office);
            }
            else if (GameStateManager.Instance.currentDay == 2)
            {
                GameStateManager.Instance.SetFlag(GameConstants.Flags.SelfTalkDay2);
                SceneStateManager.Instance.ManualSave();
                SceneManager.LoadScene(GameConstants.Scenes.Office);
            }
            else if(GameStateManager.Instance.currentDay == 3)
            {
                GameStateManager.Instance.SetFlag(GameConstants.Flags.SelfTalkDay3);
                SceneStateManager.Instance.ManualSave();
                SceneManager.LoadScene(GameConstants.Scenes.Puzzle);
            }
            else
            {
